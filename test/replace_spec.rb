...
describe Service do
  describe 'replace' do
    describe 'stores data when the server already holds data for the given key' do
      before (:each) do
        service = Service.new
      ...
      it "stores the bytes" do
        expect(@database.data_entry(@key).bytes).to eq("10")
      end
      it "stores the body" do
        expect(@database.data_entry(@key).body).to eq("Juan Perez")
      end
    ...
    it 'does not store data when empty' do
      ...
      expect(database.has_key?(key)).to be(false)
    end
    ...
    it 'does not store data if the data block does not end with \r\n' do
      ...
      database.store_data_entry("4123456", "10", "0", "10", "Juan Perez", 1)
